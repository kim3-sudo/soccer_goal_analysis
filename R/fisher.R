...
fisher.test(table(sca_nopenalty$team, sca_nopenalty$event), simulate.p.value = TRUE, B = 2000)

# Compare just manutd and colcrew
sca_mancol <- filter(sca_nopenalty, team == 'manutd' | team == 'colcrew')
fisher.test(table(sca_mancol$team, sca_mancol$event), simulate.p.value = TRUE)

# Compare just kenyon and colcrew
sca_kencol <- filter(sca_nopenalty, team == 'kenyon' | team == 'colcrew')
fisher.test(table(sca_kencol$team, sca_kencol$event), simulate.p.value = TRUE)

# Compare just kenyon and manutd
sca_kenman <- filter(sca_nopenalty, team == 'kenyon' | team == 'manutd')
fisher.test(table(sca_kenman$team, sca_kenman$event), simulate.p.value = TRUE)
...
fisher.test(table(scapen_mancol$team, scapen_mancol$event), simulate.p.value = TRUE)
# Compare just kenyon and colcrew
scapen_kencol <- filter(sca, team == 'kenyon' | team == 'colcrew')
fisher.test(table(scapen_kencol$team, scapen_kencol$event), simulate.p.value = TRUE)
# Compare just kenyon and manutd
scapen_kenman <- filter(sca, team == 'kenyon' | team == 'manutd')
fisher.test(table(scapen_kenman$team, scapen_kenman$event), simulate.p.value = TRUE)
