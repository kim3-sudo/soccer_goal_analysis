...
for i in url_list:
    ...
    for i in tqdm(matches):
        ...
        try:
            ...
            df = pd.DataFrame(data, columns = ['minute','player','squad','outcome','distance','bodypart','sca1player','sca1event','sca2player','sca2event'])
        except:
            pass

    df.to_csv(endfile + '_colcrew_' + str(year) + '.csv')
    year -= 1
